...
parser = argparse.ArgumentParser(description='Status Reporting on a complex directory structure.')
...
with open(out, 'w') as f_out:
    ...
    for d in run_dirs:
        ...
        try:
            ...
            composite_l2_masked_image_dir = os.path.join(run_dir, r"composite/cloud_masked")
        except:
            log.warning("Something went wrong with {}.".format(run_dir))

        all_sub_dirs = [composite_l1_image_dir,
        ...
        for sd in all_sub_dirs:
            try:
                dir_contents = [o for o in os.listdir(sd)    
                                      if os.path.isdir(os.path.join(sd,o))]
                '''
                ...
                file_contents = [o for o in os.listdir(sd)    
                                      if os.path.isfile(os.path.join(sd,o))]
                #log.info("  {}; {}; {}".format(os.path.basename(sd), len(dir_contents), len(file_contents)))
                line = line + ", " + str(len(dir_contents)) + ", " + str(len(file_contents))
