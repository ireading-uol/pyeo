    ...
    for index, dir_path in enumerate(dir_paths):
        ...
        if size < 10*1024:
            ...
            if size < 10*1024*1024:
                log.info("{}: {} KB  {}".format(index+1, str(round(size/1024)), dir_path))
            else:
                if size < 10*1024*1024*1024:
                    log.info("{}: {} MB  {}".format(index+1, str(round(size/1024/1024)), dir_path))
                else:
                    if size < 10*1024*1024*1024*1024:
                        log.info("{}: {} GB  {}".format(index+1, str(round(size/1024/1024/1024)), dir_path))
                    else:
                        log.info("{}: {} TB  {}".format(index+1, str(round(size/1024/1024/1024/1024)), dir_path))
    # and now each file
