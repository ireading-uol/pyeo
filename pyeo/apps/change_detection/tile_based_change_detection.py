...
def rolling_detection(config_path,
    ...
    try:
        ...
        if do_all or do_classify:
            ...
            class_image_paths = [ f.path for f in os.scandir(categorised_image_dir) if f.is_file() ]
            if len(class_image_paths) == 0:
                raise FileNotFoundError("No categorised images found in {}.".format(categorised_image_dir))

            # sort class images by detection date
            class_image_paths = \
                pyeo.filesystem_utilities.sort_by_timestamp(
                    [image_name for image_name in class_image_paths if image_name.endswith(".tif")],
                    recent_first=False
                )
